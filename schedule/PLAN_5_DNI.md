# Plan tygodniowy: piątek → niedziela → poniedziałek → wtorek
...
| Dzień | Co robi |
|-------|---------|
| **Piątek** | Discovery Serper + www → cache JSON (`de-gu-wyniki-pi`) |
...
|----------|------|----------------------|
| discovery | `de_gu_pi.yml` | `0 20 * * 5` → **20:00** piątek |
...
**Wznowienie discovery:** `gh workflow run "GU piatek discovery" -f resume_artifact_run_id=RUN_ID`
